...
function deletePO() {
	...
	try {
		...
		query = 'SELECT LIFECYCLESTATUS, APPROVALSTATUS, CONFIRMSTATUS, ORDERINGSTATUS, INVOICINGSTATUS '
				+ 'from "sap.hana.democontent.epm.data::EPM.Purchase.Header" where PURCHASEORDERID = ?';
	...
	try {
		...
		query = "UPDATE \"sap.hana.democontent.epm.data::EPM.Purchase.Header\" set LIFECYCLESTATUS = 'X' where PURCHASEORDERID = ?";
		pstmt = conn.prepareStatement(query);
		pstmt.setString(1, purchaseOrderID);
...
function approvePO() {
	...
	try {
		...
		// Update Purchase Order Status in order to delete it
		if (action === "Reject") {
			query = "UPDATE \"sap.hana.democontent.epm.data::EPM.Purchase.Header\" set APPROVALSTATUS = 'R' where PURCHASEORDERID = ?";			
		}

		if (action === "Accept") {
			query = "UPDATE \"sap.hana.democontent.epm.data::EPM.Purchase.Header\" set APPROVALSTATUS = 'A' where PURCHASEORDERID = ?";			
		}
