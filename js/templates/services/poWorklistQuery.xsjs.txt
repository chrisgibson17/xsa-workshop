...
function getFilter() {
	...
	try {
		...
		query = 'SELECT TOP 50 DISTINCT TO_NVARCHAR(COMPANYNAME) FROM "sap.hana.democontent.epm.data::EPM.MasterData.BusinessPartner" '
		...
		query = 'SELECT TOP 50 DISTINCT TO_NVARCHAR("CITY") FROM "sap.hana.democontent.epm.models::AT_BUYER" '
				+ ' WHERE CONTAINS("CITY",?)';
		pstmt = conn.prepareStatement(query);
		pstmt.setString(1, terms);
		rs = pstmt.executeQuery();
		...
		query = 'SELECT TOP 50 DISTINCT TO_NVARCHAR(CATEGORY) FROM "sap.hana.democontent.epm.data::EPM.MasterData.Products" '
		...
		// Product - Product ID
		query = 'SELECT TOP 50 DISTINCT TO_NVARCHAR(PRODUCTID) FROM "sap.hana.democontent.epm.data::EPM.MasterData.Products" '
		...
		query = 'SELECT TOP 50 DISTINCT TO_NVARCHAR("PRODUCT_NAME") FROM "sap.hana.democontent.epm.models::AT_PROD" '
		...
		query = 'SELECT TOP 50 DISTINCT TO_NVARCHAR("PRODUCT_DESCRIPTION") FROM "sap.hana.democontent.epm.models::AT_PROD" '
		...
		// PO - PO ID
		query = 'SELECT TOP 50 DISTINCT TO_NVARCHAR(PURCHASEORDERID) FROM "sap.hana.democontent.epm.data::EPM.Purchase.Header" '
...
function getTotalOrders() {
	...
	try {
		var query = 'SELECT top 5 "'
				+ ivGroupBy
				+ '", SUM("CONVGROSSAMOUNT") FROM "sap.hana.democontent.epm.models::AN_PURCHASE_COMMON_CURRENCY" (\'PLACEHOLDER\' = (\'$$IP_O_TARGET_CURRENCY$$\', \''
